...
const createWedding = async (req, res, next) => {
    try {
        const {brideName,groomName,weddingDate,venue,location,guestCount,totalBudget} = req.body
        if (!brideName || !groomName || !weddingDate) {
            return res.status(400).json({success: false,message: "Bride Name, Groom Name, and Wedding Date are required"})
        }
        ...
        return res.status(201).json({success: true,message: "Wedding created successfully",data: wedding})
    } catch (error) {
        next(error)
    }
}
