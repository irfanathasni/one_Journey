  ...
  try {
    ...
    if (booking.customer.toString() !== req.user.id.toString()) {
      return res
        ...
        .json({
          success: false,
          message: "You don't have permission for this booking",
        });
    }
    ...
    if (paymentType === "final") {
      if (booking.status !== "completed") {
        return res
          ...
          .json({
            success: false,
            message: "Event must be marked completed before final payment",
          });
      }
      if (booking.finalPaymentStatus !== "requested") {
        return res
          ...
          .json({
            success: false,
            message: "Vendor has not requested final payment yet",
          });
      }
      ...
      if (!Number.isFinite(amountToCharge) || amountToCharge <= 0) {
        return res
          .status(400)
          .json({ success: false, message: "No remaining amount to pay" });
      }
    } else {
      ...
      if (!Number.isFinite(amountToCharge) || amountToCharge <= 0) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid advance payment amount" });
      }
    }
    ...
    return res
      .status(200)
      .json({ success: true, order, keyId: process.env.RAZORPAY_KEY_ID });
  } catch (error) {
...
const verifyPayment = async (req, res, next) => {
  try {
    ...
    if (paymentType === "advance" && booking.paymentStatus === "paid") {
      return res
        .status(400)
        .json({ success: false, message: "Advance payment already verified" });
    }

    if (paymentType === "final" && booking.finalPaymentStatus === "paid") {
      return res
        .status(400)
        .json({ success: false, message: "Final payment already verified" });
    }
    ...
    if (paymentType === "final") {
      if (booking.status !== "completed") {
        return res
          ...
          .json({
            success: false,
            message: "Booking must be completed before final payment",
          });
      }
      if (booking.finalPaymentStatus !== "requested") {
        return res
          ...
          .json({
            success: false,
            message: "Final payment has not been requested",
          });
      }
      ...
      if (booking.status !== "approved") {
        return res
          ...
          .json({
            success: false,
            message: "Booking must be approved before advance payment",
          });
      }
