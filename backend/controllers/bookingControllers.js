...
const createBooking = async (req, res, next) => {
  try {
    const { weddingId, vendorId, serviceDate, startTime, endTime, packageId } =
      req.body;
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Wedding, vendor, date, start time, end time and package are required.",
      });
    }
    ...
    if (wedding.customer.toString() !== req.user.id.toString()) {
      return res.status(403).json({
        success: false,
        message: "You don't have permission to book for this wedding.",
      });
    }
    ...
    if (vendor.verificationStatus !== "approved") {
      return res
        .status(400)
        .json({ success: false, message: "This vendor is not approved" });
    }
    ...
    if (!selectedPackage) {
      return res.status(404).json({
        success: false,
        message: "Selected package not found for this vendor.",
      });
    }
    ...
    if (existingBooking) {
      return res.status(409).json({
        success: false,
        message:
          "You have already sent a booking request to this vendor for this wedding",
      });
    }
    ...
    if (!availability) {
      return res.status(409).json({
        success: false,
        message: "This time slot is not available for this vendor.",
      });
    }
...
const getVendorBookings = async (req, res, next) => {
  try {
    ...
    if (req.query.search?.trim()) {
      ...
      const [matchingUsers, matchingWeddings] = await Promise.all([
        User.find({
          name: {
            $regex: search,
            $options: "i",
      ...
      const weddingIds = matchingWeddings.map((wedding) => wedding._id);

      filter.$or = [
        { customer: { $in: customerIds } },
        { wedding: { $in: weddingIds } },
      ];
    }
  ...
  try {
    ...
    if (status === "approved") {
      if (!booking.package || !booking.package.packageId) {
        return res.status(400).json({
          success: false,
          message: "Booking package information is missing",
        });
      }
...
const completeBooking = async (req, res, next) => {
  try {
    ...
    if (booking.paymentStatus !== "paid") {
      return res.status(400).json({
        success: false,
        message: "Advance payment must be received before marking complete",
      });
    }
...
const requestFinalPayment = async (req, res, next) => {
  try {
    ...
    if (booking.status !== "completed") {
      return res.status(400).json({
        success: false,
        message: "Event must be marked completed first",
      });
    }
    if (booking.paymentStatus !== "paid") {
      return res.status(400).json({
        success: false,
        message: "Advance payment must be received first",
      });
    }
    if (booking.finalPaymentStatus === "paid") {
      return res
        .status(400)
        .json({ success: false, message: "Final payment already received" });
    }
    const remainingAmount =
      Number(booking.amount) - Number(booking.advanceAmount);
    if (!Number.isFinite(remainingAmount) || remainingAmount <= 0) {
      return res
        .status(400)
        .json({ success: false, message: "No remaining amount to request" });
    }
    if (booking.finalPaymentStatus === "requested") {
      return res.status(400).json({
        success: false,
        message: "Final payment request already sent",
      });
    }
