...
const createBooking = async (req, res, next) => {
  try {
    const { weddingId, vendorId, serviceDate, startTime, endTime ,eventType, guestCount  } = req.body

    if (!weddingId || !vendorId || !serviceDate || !startTime || !endTime) {
      return res.status(400).json({ success: false, message: "Wedding, vendor, date,start and end time are required." })
    }
...
const updateBookingStatus = async (req,res,next) => {
  try{
    ...
    if(status === "approved"){
    let finalPrice = vendor.price   // default fallback

    if (booking.eventType && booking.guestCount) {
        const matchingTier = vendor.pricing.find(p =>
            p.eventType === booking.eventType &&
            booking.guestCount >= p.minGuests &&
            booking.guestCount <= p.maxGuests
        )
        if (matchingTier) {
...
module.exports = { createBooking ,getMyBookings ,getVendorBookings,updateBookingStatus,completeBooking ,requestFinalPayment}